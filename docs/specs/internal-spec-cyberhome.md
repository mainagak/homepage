...
**2026-08-02確定(選択肢A):** Cyberhomeでは`.cgi`拡張子のファイルは自動的に実行可能
として扱われるという前提で進め、`deploy-cyberhome.yml`側にchmod処理は組み込まない。
初回GitHub Actions実行時に実機で疎通確認する(`internal-spec-testing.md`のsmoke-test
ジョブが検知できる設計になっているため、想定と異なった場合も気づける)。もし動作
しないことが判明した場合は、運用手順書への追記(旧B案)を別途検討する。
