...
**現在地:** フェーズ8(E2Eテスト)をp8-e2e-testerが独立に再実施し、**合格判定**
(2026-08-02、詳細下記)を得た。フェーズ6差し戻し対応(`site/contact.html`の
スクリプト競合修正+GA4/ロゴプレースホルダー追加、`docs/PROJECT_STATUS.md`
チェックポイント23)は、修正担当者とは別の視点で書き下ろした新規Playwright
テスト・既存自動テストスイートの実際の再実行により、独立して解消を確認できた。
**フェーズ9(最終レビュー)に着手可能。**
...
**次回再開時に最初にやること:**
1. **フェーズ9(最終レビュー・Issue確認)をp9-final-reviewerで実施する。**
   2026-08-02、p8-e2e-testerがフェーズ8(E2Eテスト)を独立に再実施し合格判定を
   得た(`docs/specs/e2e-test-report.md`「再テスト: 2026-08-02」節、
   `docs/PROJECT_STATUS.md`チェックポイント24)。フェーズ9着手にあたっては、
   下記2〜3の非ブロッキング事項(プレースホルダー実値・未追跡レガシーファイル群)を
   運営者に共有し、対応方針(フェーズ9内で扱う/フェーズ10保守サイクルへ回す)を
   確認すること。
2. (非ブロッキング、フェーズ9前に運営者確認推奨)GA4測定ID・ロゴ画像・
   `VERCEL_API_BASE_URL`・reCAPTCHA v2サイトキーは、チェックポイント23で
   TODOプレースホルダーとして実装済み(`G-XXXXXXXXXX`・
   `site/images/logo-placeholder.svg`・`REPLACE-WITH-VERCEL-PROJECT`・
   `REPLACE_WITH_RECAPTCHA_SITE_KEY`)。実測定ID・実ロゴアセット・実Vercel
   デプロイURL・実reCAPTCHAキーの入手可否を運営者に確認し、確定次第
   プレースホルダーを差し替えること。
3. `docs/specs/architecture.md`末尾の「追加質問」3〜6(非ブロッキング、Cyberhome契約
   詳細・実機確認事項)はフェーズ9以降と並行して確認する。
...
5. (非ブロッキング、繰り返し記録・未解決、フェーズ9前に運営者確認を推奨)
   リポジトリのルート直下に、`README.md`・`index.html`・`dist-release/`・
   `src/`・`public/`等の未追跡ファイルが引き続き存在する
   (`docs/PROJECT_STATUS.md`チェックポイント21・23参照)。チェックポイント23で
   中身を確認したところ、Vite製の別デザイン一式(実GA4測定ID`G-EG1WMDPTV0`を含むが、
   参考サイト調査時の不審な値と一致する内容もあり出典が不確かなため、本タスクでは
   採用しなかった)であることが判明した。git管理下にはないため通常のテスト結果には
   影響しないが、誤って参照・デプロイされるリスクがあるため、フェーズ9着手前に
   運営者が削除してよいか確認することを推奨する。
