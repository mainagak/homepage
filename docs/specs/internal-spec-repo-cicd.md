...
どこにも明示的な回答が見当たらなかった論点。ディレクトリ構成上は`site/Contents/`配下に
実ファイルを置く設計にしているが、Gitリポジトリで通常のバイナリファイルとして
コミット・バージョン管理するかどうかは未確定。

A) Gitで通常のバイナリファイルとして管理する(シンプルで分かりやすいが、ファイルを
   差し替えるたびにリポジトリ全体のサイズ・履歴が増加し続ける)。
B) `site/Contents/`配下をGit管理対象外(`.gitignore`)とし、実ファイルは常にFTPで
   直接Cyberhomeへアップロードする(リポジトリはコード・設定のみを保持し、実体
   ファイルの唯一の所在はCyberhome側になる。リポジトリの肥大化を避けられる)。
C) Git LFS(Large File Storage)で管理する(バイナリの差分管理に適するが、
   `deploy-cyberhome.yml`側でLFSのpull処理を追加する必要があり、CI構成がやや複雑
   になる)。
