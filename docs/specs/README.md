...
(2026-08-01)。`docs/specs/external-spec.md` は承認ステータス「承認」。**ただしユーザーが
承認後の仕様書に自ら加筆修正を行う予定**のため、次回再開時は必ず
`docs/specs/external-spec.md` を読み直し、承認時点(このチェックポイント)から変更が
入っていないか確認すること。変更があれば内容次第で再レビュー(フェーズ2やり直し)を
検討してからフェーズ3へ進むこと。
